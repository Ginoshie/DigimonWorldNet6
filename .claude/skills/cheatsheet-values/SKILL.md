...
- `CommitOnBackgroundClickBehavior` — `UpdateSourceTrigger=LostFocus` only commits when focus moves to a focusable element; clicking the non-focusable background doesn't. This behavior makes the root focusable (`IsTabStop=false`) and, on a `PreviewMouseDown` outside any TextBox, calls `Focus()` on the root so the edited TextBox loses focus and commits.

## Locking = Cheat-Engine freeze
While `IsLocked`, `Refresh()` skips the read (display stays put). A dedicated `Observable.Interval(FREEZE_INTERVAL_MS = 100ms)` loop in `CheatSheetViewModel` (separate from the 1s read poll; both in its `CompositeDisposable`) calls `PushLockedValueToMemory()` on each lockable, re-asserting the held value (`_lastNotifiedValue`). **`PushLockedValueToMemory()` must NOT short-circuit on `IsEditing`** — it writes the stored value to RAM, not the in-progress textbox text, so a locked field has to keep freezing even while focused (the `IsEditing` guard belongs only on `Refresh()`, which reads). Runtime-only — no persistence. A poll-based freeze can't win a sub-frame race (e.g. a value the game consumes the same frame it writes it); that's a known limit.
